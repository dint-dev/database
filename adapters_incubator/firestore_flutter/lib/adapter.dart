...
/// This adapters enables [package:database](https://github.com/dint-dev/database)
...
Object _valueToDart(Database database, Object argument) {
  if (argument == null ||
      argument is bool ||
      argument is num ||
      argument is DateTime ||
  ...
  if (argument is List) {
    return List.unmodifiable(
      argument.map((item) => _valueToDart(database, item)),
    );
  }
  if (argument is Map) {
    final result = <String, Object>{};
    for (var entry in argument.entries) {
      result[entry.key as String] = _valueToDart(database, entry.value);
    }
...
class FirestoreFlutter extends DatabaseAdapter {
  ...
  Stream<Snapshot> performRead(ReadRequest request) async* {
    ...
    yield (Snapshot(
      ...
      data: _valueToDart(request.document.database, implSnapshot.data),
    ));
  }
    ...
    final result = fsQuery.snapshots().map((implSnapshot) {
      final snapshots = implSnapshot.documents.map((implSnapshot) {
        return Snapshot(
          ...
          data: _valueToDart(request.collection.database, implSnapshot.data),
        );
      });
      return QueryResult(
