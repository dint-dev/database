...
import 'protos/datastore.pbgrpc.dart' as pb;

pb.Collection grpcCollectionFromDart(Collection collection) {
  return pb.Collection()..collectionId = collection.collectionId;
}

Collection grpcCollectionToDart(Datastore datastore, pb.Collection argument) {
  return datastore.collection(argument.collectionId);
}
...
Document grpcDocumentToDart(Datastore datastore, pb.Document argument) {
  return datastore
...
pb.Error grpcErrorFromDart(Object argument) {
  if (argument is DatastoreException) {
    return pb.Error()
...
Object grpcErrorToDart(pb.Error argument) {
  return DatastoreException.custom(
