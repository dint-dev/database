...
import 'package:datastore/src/adapters_framework/write_request.dart';
...
/// }
/// ```
class MemoryDatastore extends DatastoreAdapter {
  ...
  /// Latency for simulating latency in slower datastores.
  ...
  Future<void> performWrite(WriteRequest request) {
    ...
    switch (request.type) {
      case WriteType.delete:
        if (!exists) {
          return Future<void>.error(DatastoreException.notFound(document));
        }
        map.remove(key);
        break;
      ...
      case WriteType.insert:
        if (exists) {
          return Future<void>.error(DatastoreException.found(document));
        }
